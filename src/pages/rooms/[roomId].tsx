...
const MessageList = forwardRef<HTMLDivElement, {messages: Message[]; session: Session}>((props, ref) => {
    ...
    return (
        <>
            <Head>
                <title> {`${newMessage ? `(${newMessage}) ` : ''}Ding! Messenger`}</title>
            ...
            >
                ...
                {messages.slice(0, -1).map(({id, message, sentAt, sender}) => {
                    ...
                    return (
                        <div
                            className={`group w-fit flex items-center gap-1 ${
                                sender === session.user?.name ? 'self-end' : ''
                            }`}
                            key={id}
                        >
                            ...
                            <div
                                className={`snap-end flex gap-2 max-w-prose items-center rounded-xl break-words w-fit p-2 ${
                                    sender === session.user?.name
                ...
                {messages.at(-1) && (
                    <div
                        className={`group w-fit flex gap-1 items-center ${
                            messages.at(-1)?.sender === session.user?.name ? 'self-end' : ''
                        }`}
                    >
                        ...
                        <div
                            ...
                            className={`snap-end flex gap-2 rounded-xl items-center max-w-prose w-fit p-2 break-words ${
                                messages.at(-1)?.sender === session.user?.name
                                    ? 'animate-slide-from-right rounded-br-sm bg-primary text-light'
